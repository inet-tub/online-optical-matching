...
for trace in traces:
    markerindex=0
    print(trace)
    dft = df[(df['trace']==trace)&(df['alg']!="pred")&(df['alg']!="pred-history")&(df['alg']!="oblivious-2")&(df['alg']!="oblivious-4")&(df['alg']!="oblivious-16")&(df['alg']!="oblivious-64")&(df["alpha"]!=0)&(df['alpha']<=1024)]
...
for pred_alg, plot_prefix, display_name in pred_algs:
    for trace in traces:
        markerindex=0
